...
def getJD(month, day):
    ...
    return sum(monthDays[:month]) + day
...
def parse_ladybug_analysis_period(analysis_period):
    if analysis_period is not None:
        ...
        start_doy = int(getJD(st_month, st_day))
        end_doy = int(getJD(end_month, end_day))
...
#         stJD = getJD(month, 1)
#         endJD = getJD(month, monthDays[month])
#         monthlyData = temperatureData[getHour(stJD, 1)-1: getHour(endJD, 24)]
...
def set_uwg_input(uwg, DFCity, epw_site_par, bnd_layer_par, analysis_period, simTimestep=300):
